...
const DIAGNOSTICS = {
  502: {
    ...
    likelyCause: 'The Base44 backend could not reach the VPS bridge endpoint. The tunnel, reverse proxy, or VPS service is not reachable from the Base44 cloud environment.',
    endpointNote: 'Target: VERIDAN_BRIDGE_URL + /api/obsidian/dry-run  (token hidden)',
    checks: [
      'Confirm the VPS bridge service is running on the VPS',
      'Confirm VERIDAN_BRIDGE_URL is set correctly in App Secrets (no trailing slash, correct port)',
      'Check if a Cloudflare Tunnel / ngrok / reverse proxy is active and forwarding traffic',
      'Verify the VPS firewall allows inbound connections on the bridge port',
      'Test the endpoint manually from a server context (not browser)',
    ],
  },
  503: {
    icon: AlertTriangle,
