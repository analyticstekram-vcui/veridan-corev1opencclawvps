/**
 * RegisteredAgentWorkflow — Planning-only registered agent workflow placeholder.
 * localStorage only. No registered agent API calls. No legal filing. No credential storage.
 */
  ...
  return (
    <div className="space-y-4 font-mono">
      <div>
        ...
        <div className="text-[8px] text-slate-500 mt-0.5">Plan registered agent steps · No API calls · No legal filing · No credential storage</div>
      </div>

      <div className="flex items-start gap-2 px-4 py-3 bg-destructive/5 border border-destructive/30 rounded-sm">
        <AlertTriangle className="w-3.5 h-3.5 text-destructive shrink-0 mt-0.5" />
        <div>
          <div className="text-[9px] font-bold text-destructive mb-0.5">Planning only. No registered agent API calls occur from this system.</div>
          <div className="text-[8px] text-destructive/70">No legal filing · No payment processing · No registered agent API calls · No credential storage</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-1.5">
        {[
          ...
          { label: 'Registered Agent API',      value: 'DISABLED' },
      ...
      <div className="px-4 py-6 bg-card border border-border/40 rounded-sm text-center">
        <div className="text-[9px] font-bold text-slate-400 mb-1">Registered Agent Workflow — Coming Soon</div>
        <div className="text-[8px] text-slate-600">
          Future: Track registered agent selection, service provider research, state compliance steps, and annual report reminders — all localStorage-only.
        </div>
      </div>
