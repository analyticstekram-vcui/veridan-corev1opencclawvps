/**
 * BrowserObservationContractValidatorFinalLockPanel — Local-only Phase 18 Final Lock
 * Verifies the backend validator result and frontend validation panel.
...
const LOCK_KEY      = 'openclawBrowserObservationContractValidatorFinalLock';
const LOCK_NAME     = 'OPENCLAW_BROWSER_OBSERVATION_CONTRACT_VALIDATOR_FINAL_LOCK';
const PHASE_NAME    = 'PHASE_18_DRY_RUN_OBSERVATION_CONTRACT_VALIDATOR';
const FINAL_WARNING = 'This Phase 18 lock is local-only and non-executable. It confirms dry-run validation only. It does not authorize OpenClaw calls, browser automation, clicking, typing, credentials, trading, broker actions, wallet actions, money movement, command dispatch, scheduler, polling, or external forwarding.';
...
const CHECK_LABELS = {
  phase17FinalLockPresent:        'Phase 17 final lock present',
...
const VALIDATION_STATUS_CONFIG = {
  VALID_DRY_RUN:        { color: 'text-primary',     badge: 'text-primary border-primary/30 bg-primary/5' },
  REJECTED_BY_CONTRACT: { color: 'text-amber-500',   badge: 'text-amber-500 border-amber-500/30 bg-amber-500/5' },
  BLOCKED_BY_POLICY:    { color: 'text-destructive', badge: 'text-destructive border-destructive/30 bg-destructive/5' },
};
...
function runLock() {
  ...
  const lockChecks = {
    phase17FinalLockPresent:             !!phase17Lock,
...
export default function BrowserObservationContractValidatorFinalLockPanel() {
  const [lock, setLock]     = useState(() => loadJSON(LOCK_KEY, null));
  const [copied, setCopied] = useState(false);

  const phase17Present    = !!localStorage.getItem(KEYS.phase17Lock);
  const valResultPresent  = !!localStorage.getItem(KEYS.validationResult);
  ...
  const SOURCE_PRESENCE = [
    { label: 'Phase 17 Final Lock', present: phase17Present },
    { label: 'Validation Result',   present: valResultPresent },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Header */}
      <div>
        <div className="text-[11px] uppercase tracking-widest text-slate-400 font-semibold mb-0.5">Phase 18 · Contract Validator Final Lock</div>
        <div className="text-[13px] font-bold text-foreground flex items-center gap-2">
          <Lock className="w-4 h-4 text-primary" /> Contract Validator Final Lock
        </div>
        <div className="text-[9px] text-slate-500 mt-0.5">Local-only Phase 18 final lock. Verifies dry-run validator integrity. No OpenClaw calls, no browser automation, no execution.</div>
      </div>
      ...
      <div>
        <div className="text-[9px] uppercase tracking-widest text-slate-400 font-semibold mb-2">
          Source Packets — {SOURCE_PRESENCE.filter(s => s.present).length}/{SOURCE_PRESENCE.length} Present
        </div>
        <div className="grid grid-cols-2 gap-2">
          {SOURCE_PRESENCE.map(({ label, present }) => (
      ...
      {valResult && (
        <div className="bg-card border border-border rounded-lg px-4 py-3 flex items-center justify-between">
          ...
          <span className={`text-[9px] font-bold px-2 py-1 rounded border uppercase tracking-wider ${
            VALIDATION_STATUS_CONFIG[valResult.validationStatus]?.badge ?? 'text-slate-400 border-slate-500/30 bg-slate-500/5'
          }`}>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          ...
          {copied ? 'Copied!' : 'Copy Phase 18 Final Lock JSON'}
        </button>
        <button
          type="button"
          onClick={handleClear}
        ...
        >
          ...
          Clear Local Phase 18 Final Lock
        </button>
      </div>
      ...
      {lock && (
        <div className="bg-secondary/10 border border-border/60 rounded-lg overflow-hidden">
          <div className="flex items-center justify-between px-4 py-2 bg-secondary/20 border-b border-border">
            <span className="text-[9px] uppercase tracking-widest text-slate-400 font-semibold">Phase 18 Final Lock — JSON Preview</span>
