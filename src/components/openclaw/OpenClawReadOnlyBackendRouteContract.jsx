/**
 * OpenClawReadOnlyBackendRouteContract — Phase 38
 * Defines local-only backend route contracts for future controlled read-only connector checks.
 * No OpenClaw calls, no backend APIs, no execution. Route contract design only.
 */
...
const PERMITTED_ENDPOINTS = ['/health', '/status', '/version', '/capabilities'];
...
function generateRouteContract(testPlan) {
  return {
    routeContractId: `contract-${testPlan.testPlanId}-${Date.now()}`,
    sourceTestPlanId: testPlan.testPlanId,
    sourceConnectorValidationId: testPlan.sourceConnectorValidationId,
    sourceConnectorContractId: testPlan.sourceConnectorContractId,
    sourceRequestId: testPlan.sourceRequestId,
    generatedAt: new Date().toISOString(),
    routeMode: 'READ_ONLY_BACKEND_CONTRACT',
    proposedBackendRoute: '/api/openclaw/read-only/status-check',
    allowedMethod: 'POST_TO_BACKEND_ONLY',
    backendActionMode: 'SERVER_SIDE_READ_ONLY_PROXY',
    permittedOpenClawEndpoints: PERMITTED_ENDPOINTS,
    permittedOpenClawMethod: 'GET',
    blockedBackendActions: BLOCKED_ACTIONS,
...
export default function OpenClawReadOnlyBackendRouteContract() {
  ...
  const handleGenerateRouteContract = () => {
    try {
      const testPlanBatches = loadJSON(TEST_PLAN_KEY, []);

      if (testPlanBatches.length === 0) {
        setLastAction('No Phase 37 test plans found');
        return;
      }

      const latestBatch = testPlanBatches[0];

      if (!latestBatch.testPlans || latestBatch.testPlans.length === 0) {
        setLastAction('No test plans in latest batch');
        return;
      }

      // Filter for valid test plans
      const validPlans = latestBatch.testPlans.filter(
        plan =>
          plan.testPlanMode === 'READ_ONLY_TEST_PLAN' &&
          plan.allowedMethod === 'GET' &&
          plan.allowedEndpoints &&
          plan.allowedEndpoints.includes('/health') &&
          plan.allowedEndpoints.includes('/status') &&
          plan.allowedEndpoints.includes('/version') &&
          plan.allowedEndpoints.includes('/capabilities') &&
      ...
      if (validPlans.length === 0) {
        setLastAction('No valid test plans found (all safety flags must match READ_ONLY_TEST_PLAN specification)');
        return;
      }
      ...
      const contractBatch = {
        ...
        contractType: 'PHASE_38_READONLY_BACKEND_ROUTE_CONTRACT',
        generatedAt: new Date().toISOString(),
        sourceTestPlanBatchId: latestBatch.testPlanBatchId,
        totalRouteContracts: generatedContracts.length,
        routeContracts: generatedContracts,
      };
      ...
      setLastAction(`Generated ${generatedContracts.length} route contracts from valid test plans`);
    } catch (err) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Header */}
      <div>
        <div className="text-[11px] uppercase tracking-widest text-slate-400 font-semibold mb-0.5">Phase 38 · Read-Only Backend Route Contract</div>
        <div className="text-[13px] font-bold text-foreground flex items-center gap-2">
          <ShieldCheck className="w-4 h-4 text-primary" /> Read-Only Backend Route Contract
        </div>
        <div className="text-[9px] text-slate-500 mt-0.5">Defines local-only backend route contracts for future controlled read-only connector checks without calling anything.</div>
      </div>

      {/* Phase info chip */}
      <div className="px-3 py-1.5 bg-primary/5 border border-primary/20 rounded inline-flex items-center gap-1.5">
        <ShieldCheck className="w-3 h-3 text-primary shrink-0" />
        <span className="text-[9px] font-mono font-bold text-primary">PHASE_38_READONLY_BACKEND_ROUTE_CONTRACT</span>
      </div>
      ...
      {latestBatch && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          ...
          <div className="bg-card border border-border rounded-lg px-4 py-3">
            <div className="text-[8px] uppercase tracking-widest text-slate-500 font-semibold mb-1">Permitted Endpoints</div>
            <div className="text-[18px] font-bold text-primary">{PERMITTED_ENDPOINTS.length}</div>
          </div>
          <div className="bg-card border border-border rounded-lg px-4 py-3">
            <div className="text-[8px] uppercase tracking-widest text-slate-500 font-semibold mb-1">Blocked Actions</div>
            <div className="text-[18px] font-bold text-destructive">{BLOCKED_ACTIONS.length}</div>
          </div>
          <div className="bg-card border border-border rounded-lg px-4 py-3">
        ...
        <div className="space-y-2 px-4 py-3">
          <div className="text-[8px] space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-slate-300">Mode: <span className="font-mono font-semibold text-primary">READ_ONLY_BACKEND_CONTRACT</span></span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle2 className="w-3 h-3 text-primary shrink-0" />
              <span className="text-slate-300">Backend Route: <span className="font-mono font-semibold text-primary">/api/openclaw/read-only/status-check</span></span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle2 className="w-3 h-3 text-primary shrink-0" />
              <span className="text-slate-300">Action Mode: <span className="font-mono font-semibold text-primary">SERVER_SIDE_READ_ONLY_PROXY</span></span>
            </div>
            <div className="flex items-start gap-2">
              <CheckCircle2 className="w-3 h-3 text-primary shrink-0 mt-0.5" />
              <span className="text-slate-300">Permitted OpenClaw Endpoints: <span className="font-mono font-semibold text-primary">{PERMITTED_ENDPOINTS.join(', ')}</span></span>
            </div>
            <div className="flex items-start gap-2">
              <CheckCircle2 className="w-3 h-3 text-destructive shrink-0 mt-0.5" />
              <span className="text-slate-300">Blocked Backend Actions: <span className="font-mono font-semibold text-destructive">{BLOCKED_ACTIONS.join(', ')}</span></span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          ...
          Generate Read-Only Backend Route Contract
        </button>
        <button
          type="button"
          onClick={handleCopy}
      ...
      {latestBatch && latestBatch.routeContracts && latestBatch.routeContracts.length > 0 && (
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          <div className="px-4 py-2 bg-secondary/20 border-b border-border">
            <span className="text-[9px] uppercase tracking-widest text-slate-400 font-semibold">Route Contracts ({latestBatch.routeContracts.length})</span>
          </div>
            ...
            <table className="w-full text-[8px]">
              <thead className="bg-secondary/10 border-b border-border/30">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left font-semibold text-slate-400 uppercase tracking-wider">Route Mode</th>
                  <th className="px-3 py-2 text-left font-semibold text-slate-400 uppercase tracking-wider">Backend Route</th>
              ...
              <tbody className="divide-y divide-border/20">
                {latestBatch.routeContracts.map((contract, i) => (
                  <tr key={i} className="hover:bg-secondary/10 transition-colors">
                    <td className="px-3 py-2.5 text-slate-300 font-mono text-[7px] truncate">{contract.sourceRequestId}</td>
                    <td className="px-3 py-2.5 text-primary font-semibold text-[8px]">READ_ONLY</td>
                    <td className="px-3 py-2.5 text-slate-300 font-mono text-[7px] truncate">{contract.proposedBackendRoute}</td>
      ...
      <div className="bg-primary/5 border-2 border-primary/20 rounded-lg p-4 space-y-2">
        ...
        <p className="text-[9px] text-slate-300 leading-relaxed">
          This is a backend route contract only. It does NOT call OpenClaw, invoke backend functions, dispatch commands, trade, enter credentials, schedule tasks, poll, use browser automation, use wallets, or move money. Route contract design is local-only, non-executable design work.
        </p>
        <div className="pt-2 border-t border-primary/10 text-[8px] text-slate-400 space-y-0.5">
          <div className="flex items-center gap-1.5">
            <CheckCircle2 className="w-3 h-3 text-primary shrink-0" />
            <span>This is a backend route contract only</span>
          </div>
          <div className="flex items-center gap-1.5">
            <CheckCircle2 className="w-3 h-3 text-primary shrink-0" />
      ...
      <div className="flex items-center gap-2 px-3 py-2 bg-primary/5 border border-primary/20 rounded text-[8px] text-primary/80">
        ...
        Local-only route contract. No fetch, no OpenClaw calls, no backend calls, no execution, no dispatch.
      </div>
    </div>
  );
}