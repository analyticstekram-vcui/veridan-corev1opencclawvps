/**
 * BaselineExportPacketPanel — Local-only Baseline Export Packet
 * Bundles current baseline archive into one copyable/exportable JSON packet.
 * No backend calls, no OpenClaw calls, no fetch, no execution, no dispatch.
 */
import React, { useState } from 'react';
import { Download, Copy, CheckCircle2, Trash2, ShieldCheck, AlertTriangle } from 'lucide-react';
...
const EXPORT_NAME  = 'OPENCLAW_READ_ONLY_MONITORING_EXPORT_PACKET';
const BASELINE_NAME = 'OPENCLAW_READ_ONLY_MONITORING_STABLE_BASELINE';
const EXPORT_STATUS = 'LOCAL_ONLY_EXPORT_READY';
...
export default function BaselineExportPacketPanel() {
  const [exportPacket, setExportPacket] = useState(() => loadJSON(EXPORT_KEY));
  ...
  const handleGenerate = () => {
    ...
    const p = {
      ...
      baselineArchiveManifest: manifest ?? null,
    ...
    try { localStorage.setItem(EXPORT_KEY, JSON.stringify(p, null, 2)); } catch {}
    setExportPacket(p);
  };

  const handleCopy = () => {
    if (!exportPacket) return;
    navigator.clipboard.writeText(JSON.stringify(exportPacket, null, 2));
    setCopied(true);
  ...
  const assertionCount = exportPacket ? Object.keys(exportPacket.safetyAssertions).length : 0;
  const assertionPassCount = exportPacket ? Object.values(exportPacket.safetyAssertions).filter(Boolean).length : 0;
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Header */}
      <div>
        <div className="text-[11px] uppercase tracking-widest text-slate-400 font-semibold mb-0.5">Export Packet</div>
        <div className="text-[13px] font-bold text-foreground flex items-center gap-2">
          <Download className="w-4 h-4 text-primary" /> Baseline Export Packet
        </div>
        <div className="text-[9px] text-slate-500 mt-0.5">Bundled export of baseline archive. Local-only. No backend calls. No OpenClaw calls. No execution.</div>
      </div>

      {/* Export name + Status chips */}
      <div className="flex flex-wrap gap-2">
        ...
        <div className="px-3 py-1.5 bg-secondary/20 border border-border rounded">
          <span className="text-[9px] font-mono font-bold text-slate-300">{EXPORT_STATUS}</span>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        ...
        <div className="divide-y divide-border/30">
          {[
            ...
            { label: 'Baseline Archive Manifest Key', value: MANIFEST_KEY, present: !!localStorage.getItem(MANIFEST_KEY) },
      ...
      <div className="flex items-start gap-2 px-3 py-2.5 bg-amber-500/5 border border-amber-500/20 rounded">
        ...
        <p className="text-[9px] text-amber-500/90">
          <span className="font-bold">Export Packet is local-only and non-executable.</span>{' '}
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button
          type="button"
          onClick={handleCopy}
          disabled={!exportPacket}
          className="flex items-center gap-1.5 px-3 py-2 text-[10px] border border-border text-slate-400 hover:text-foreground hover:bg-secondary/50 rounded font-bold transition-colors disabled:opacity-40"
        >
          {copied ? <CheckCircle2 className="w-3.5 h-3.5 text-primary" /> : <Copy className="w-3.5 h-3.5" />}
          {copied ? 'Copied!' : 'Copy Export Packet JSON'}
        </button>
        <button
          type="button"
          onClick={handleClear}
        ...
        <div className="bg-secondary/10 border border-border/60 rounded-lg overflow-hidden">
          <div className="flex items-center justify-between px-4 py-2 bg-secondary/20 border-b border-border">
            <span className="text-[9px] uppercase tracking-widest text-slate-400 font-semibold">
              Export Packet — Safety Assertions: {assertionPassCount}/{assertionCount} PASS
            </span>
            <span className="text-[8px] font-mono text-slate-500">{new Date(exportPacket.generatedAt).toLocaleString()}</span>
          </div>
