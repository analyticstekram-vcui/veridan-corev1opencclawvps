...
const PHASES = [
  ...
  {
    phase: 42,
    title: 'Backend Presence Check Route Validator',
    plainEnglish: 'Validates Phase 41 backend presence check route contracts locally before any future backend implementation. PASS only means eligible for future backend implementation, not execution.',
    built: true,
    required: false,
    storageKey: 'openclawPhase42BackendPresenceCheckRouteValidationResults',
    safetyStatus: 'LOCKED',
    safetyColor: 'text-primary',
  },
  {
  ...
  {
    phase: 45,
    title: 'Backend Presence Check Activation Lock',
    plainEnglish: 'Final local-only activation lock before any real backend environment presence check route is implemented. Prepares the checkpoint for Phase 46. Does not access process.env, read secrets, or execute anything.',
    built: true,
    required: false,
    storageKey: 'openclawPhase45BackendPresenceCheckActivationLocks',
    safetyStatus: 'LOCKED',
    safetyColor: 'text-primary',
  },
  {
    phase: 46,
    title: 'Backend Env Presence Boolean Route',
    plainEnglish: 'First controlled backend presence check. The backend route checks whether required OpenClaw env keys exist and returns only boolean presence. Secret values are never returned, logged, displayed, or stored.',
    built: true,
    required: false,
    storageKey: 'openclawPhase46BackendEnvPresenceBooleanResults',
    safetyStatus: 'BACKEND_BOOLEAN_ONLY',
    safetyColor: 'text-primary',
  },
  {
    phase: 47,
    title: 'Backend Env Presence Evidence Record',
    plainEnglish: 'Records backend boolean presence evidence from Phase 46 results before any OpenClaw health check is allowed. Evidence records show key presence summary, redaction verified, and next allowed phase. No secret values, no OpenClaw calls.',
    built: true,
    required: false,
    storageKey: 'openclawPhase47BackendEnvPresenceEvidenceRecords',
    safetyStatus: 'BACKEND_BOOLEAN_EVIDENCE',
    safetyColor: 'text-primary',
  },
  {
    phase: 48,
  ...
  {
    phase: 49,
    title: 'OpenClaw Health Check Activation Lock',
    plainEnglish: 'Final local-only activation lock before the first real OpenClaw read-only health check route is implemented. Prepares the checkpoint for Phase 50. Does not call OpenClaw, dispatch commands, access secrets, or execute anything.',
    built: true,
    required: false,
    storageKey: 'openclawPhase49HealthCheckActivationLocks',
    safetyStatus: 'LOCKED',
    safetyColor: 'text-primary',
  },
  {
    phase: 50,
    title: 'OpenClaw Read-Only Health Check Route',
    plainEnglish: 'First controlled read-only OpenClaw call. Backend calls only GET /health. Returns boolean/status summary only. Secret values never returned. No dispatch, execution, trading, browser automation, scheduler, polling, wallet, or money movement.',
    built: true,
    required: false,
    storageKey: 'openclawPhase50OpenClawReadOnlyHealthCheckResults',
    safetyStatus: 'OPENCLAW_READ_ONLY_HEALTH',
    safetyColor: 'text-primary',
  },
  {
    phase: 51,
    title: 'OpenClaw Health Check Evidence Record',
    plainEnglish: 'Records evidence from Phase 50 health check results. Validates all 15 safety conditions before creating evidence records. Does not call OpenClaw, access secrets, dispatch commands, or execute anything.',
    built: true,
    required: false,
    storageKey: 'openclawPhase51HealthCheckEvidenceRecords',
    safetyStatus: 'OPENCLAW_READ_ONLY_EVIDENCE',
    safetyColor: 'text-primary',
  },
  {
    phase: 52,
    title: 'OpenClaw Status / Version / Capabilities Contract',
    plainEnglish: 'Defines a local-only contract for the next read-only OpenClaw expansion — /status, /version, /capabilities endpoints. Does not call OpenClaw, access secrets, dispatch commands, or execute anything. Contract only.',
    built: true,
    required: false,
    storageKey: 'openclawPhase52StatusVersionCapabilitiesContracts',
    safetyStatus: 'LOCKED',
    safetyColor: 'text-primary',
  },
  {
    phase: 53,
    title: 'OpenClaw Status / Version / Capabilities Activation Lock',
    plainEnglish: 'Final local-only activation lock before the status/version/capabilities backend route is implemented. Validates Phase 52 contracts. Does not call OpenClaw, access secrets, dispatch commands, or execute anything.',
    built: true,
    required: false,
    storageKey: 'openclawPhase53StatusVersionCapabilitiesActivationLocks',
    safetyStatus: 'LOCKED',
    safetyColor: 'text-primary',
  },
  {
  ...
  {
    phase: 55,
    title: 'OpenClaw Status / Version / Capabilities Evidence Record',
    plainEnglish: 'Records evidence from Phase 54 read-only results for /status, /version, /capabilities. Validates all safety flags before creating evidence records. Does not call OpenClaw, access secrets, dispatch commands, or execute anything.',
    built: true,
    required: false,
    storageKey: 'openclawPhase55StatusVersionCapabilitiesEvidenceRecords',
    safetyStatus: 'OPENCLAW_READ_ONLY_SVC_EVIDENCE',
    safetyColor: 'text-primary',
  },
  {
    phase: 56,
    title: 'OpenClaw Read-Only Capability Policy Map',
    plainEnglish: 'Local-only policy map classifying all read-only capabilities allowed through Phase 55 evidence. Defines allowed and blocked capabilities. Introduces no new network route. Does not call OpenClaw.',
    built: true,
    required: false,
    storageKey: 'openclawPhase56ReadOnlyCapabilityPolicyMaps',
    safetyStatus: 'READ_ONLY_POLICY_MAP',
    safetyColor: 'text-primary',
  },
  {
    phase: 'FUTURE',
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="text-[9px] text-slate-500 mt-0.5">Plain English view of phases 26–56 and future gates. How it all connects.</div>
      </div>
      ...
      <div className="flex items-center gap-2 px-4 py-3 bg-primary/10 border border-primary/20 rounded-lg">
        ...
        <div>
          ...
          <div className="text-[10px] text-primary mt-0.5">Phase 56 Complete — OpenClaw Read-Only Capability Policy Map Installed</div>
        </div>
      </div>
