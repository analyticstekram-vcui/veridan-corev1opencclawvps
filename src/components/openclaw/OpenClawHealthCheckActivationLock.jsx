/**
 * OpenClawHealthCheckActivationLock — Phase 49
 * Final activation lock before the first real OpenClaw read-only health check route is implemented.
...
const PHASE48_CONTRACTS_KEY = 'openclawPhase48HealthCheckContracts';
const ACTIVATION_LOCKS_KEY = 'openclawPhase49HealthCheckActivationLocks';
...
function isValidPhase48Contract(c) {
  if (!c) return false;
  if (c.contractMode !== 'OPENCLAW_HEALTH_CHECK_CONTRACT') return false;
  if (c.proposedBackendRoute !== '/api/openclaw/read-only/health-check') return false;
  if (c.proposedBackendMethod !== 'POST') return false;
  if (c.openClawEndpoint !== '/health') return false;
  if (c.openClawMethod !== 'GET') return false;
  if (c.allowedPurpose !== 'READ_ONLY_HEALTH_CHECK') return false;
  if (c.requiredEnvPresenceEvidence !== true) return false;
...
function generateActivationLock(contract) {
  return {
    healthCheckActivationLockId: `lock49-${contract.healthCheckContractId}-${Date.now()}`,
    sourceHealthCheckContractId: contract.healthCheckContractId,
    sourceEvidenceRecordId: contract.sourceEvidenceRecordId,
    sourcePhase46ResultId: contract.sourcePhase46ResultId,
    sourceActivationLockId: contract.sourceActivationLockId,
    generatedAt: new Date().toISOString(),
    activationLockMode: 'OPENCLAW_HEALTH_CHECK_ACTIVATION_LOCK',
    lockedBackendRoute: '/api/openclaw/read-only/health-check',
    lockedOpenClawEndpoint: '/health',
    lockedOpenClawMethod: 'GET',
    nextAllowedPhase: 'PHASE_50_OPENCLAW_READ_ONLY_HEALTH_CHECK_ROUTE',
    ...
    allowedFutureBackendBehavior: 'READ_ONLY_OPENCLAW_HEALTH_CHECK',
    allowedFutureResponseShape: {
      routeStatus: 'string',
      checkedAt: 'ISO_TIMESTAMP',
      openClawReachable: 'true/false',
...
export default function OpenClawHealthCheckActivationLock() {
  const [locks, setLocks] = useState(() => loadJSON(ACTIVATION_LOCKS_KEY, []));
  ...
  const handleGenerate = () => {
    try {
      const contractBatches = loadJSON(PHASE48_CONTRACTS_KEY, []);

      if (contractBatches.length === 0) {
        setLastAction('No Phase 48 contracts found — generate health check contracts first');
        return;
      }

      const allContracts = contractBatches.flatMap(b => b.contracts || []);
      const validContracts = allContracts.filter(isValidPhase48Contract);

      if (validContracts.length === 0) {
        setLastAction('No valid Phase 48 contracts found — all 17 gate conditions must be satisfied');
        return;
      }
      ...
      const batch = {
        ...
        batchType: 'PHASE_49_OPENCLAW_HEALTH_CHECK_ACTIVATION_LOCK',
        generatedAt: new Date().toISOString(),
        totalActivationLocks: generated.length,
        activationLocks: generated,
      };

      try {
        localStorage.setItem(ACTIVATION_LOCKS_KEY, JSON.stringify([batch, ...locks].slice(0, 50)));
      } catch {}

      setLocks([batch, ...locks].slice(0, 50));
      setLastAction(`Generated ${generated.length} activation locks from valid Phase 48 contracts`);
    } catch (err) {
  ...
  const handleClear = () => {
    try {
      localStorage.removeItem(ACTIVATION_LOCKS_KEY);
      setLocks([]);
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Header */}
      <div>
        <div className="text-[11px] uppercase tracking-widest text-slate-400 font-semibold mb-0.5">Phase 49 · OpenClaw Health Check Activation Lock</div>
        <div className="text-[13px] font-bold text-foreground flex items-center gap-2">
          <ShieldCheck className="w-4 h-4 text-primary" /> OpenClaw Health Check Activation Lock
        </div>
        <div className="text-[9px] text-slate-500 mt-0.5">Final activation lock before the first real OpenClaw read-only health check route is implemented. Does not call OpenClaw.</div>
      </div>

      {/* Phase info chip */}
      <div className="px-3 py-1.5 bg-primary/5 border border-primary/20 rounded inline-flex items-center gap-1.5">
        <ShieldCheck className="w-3 h-3 text-primary shrink-0" />
        <span className="text-[9px] font-mono font-bold text-primary">PHASE_49_OPENCLAW_HEALTH_CHECK_ACTIVATION_LOCK</span>
      </div>
      ...
      {latestLock && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          ...
          <div className="bg-card border border-border rounded-lg px-4 py-3">
            ...
            <div className="text-[10px] font-mono text-amber-500">{latestLock.lockedOpenClawEndpoint}</div>
          </div>
          <div className="bg-card border border-border rounded-lg px-4 py-3">
            <div className="text-[8px] uppercase tracking-widest text-slate-500 font-semibold mb-1">Generated</div>
            <div className="text-[9px] font-mono text-slate-300">{new Date(latestBatch.generatedAt).toLocaleTimeString()}</div>
          </div>
        </div>
      )}
        ...
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          ...
          <div className="px-4 py-3 border-b border-border/30 grid grid-cols-2 gap-3 text-[8px]">
            {[
              ...
              ['lockedOpenClawEndpoint', latestLock.lockedOpenClawEndpoint],
              ...
              ['actualExecutionStatus', latestLock.actualExecutionStatus],
            ].map(([label, val]) => (
          ...
          <div className="px-4 py-3 grid grid-cols-2 sm:grid-cols-3 gap-2 text-[8px]">
            {[
              ...
              ['dryRunOnly', latestLock.dryRunOnly],
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          ...
          Generate OpenClaw Health Check Activation Lock
        </button>
        <button
          type="button"
          onClick={handleCopy}
              ...
              <tbody className="divide-y divide-border/20">
                {latestBatch.activationLocks.map((lock, i) => (
                  <tr key={i} className="hover:bg-secondary/10 transition-colors">
                    <td className="px-3 py-2.5 text-slate-300 font-mono text-[7px] truncate max-w-[100px]">{lock.healthCheckActivationLockId}</td>
                    <td className="px-3 py-2.5 text-amber-500 font-bold text-[7px]">{lock.activationStatus}</td>
                    <td className="px-3 py-2.5 text-primary font-bold text-[7px] truncate max-w-[100px]">{lock.nextAllowedPhase}</td>
          ...
          {expandedLock !== null && latestBatch.activationLocks[expandedLock] && (
            <div className="bg-secondary/10 border-t border-border p-4 space-y-2">
              <div className="text-[9px] font-semibold text-primary">
                Activation Lock — {latestBatch.activationLocks[expandedLock].healthCheckActivationLockId}
              </div>
      ...
      <div className="bg-primary/5 border-2 border-primary/20 rounded-lg p-4 space-y-2">
        ...
        <div className="pt-1 text-[8px] text-slate-400 space-y-0.5">
          {[
            ...
            'It only prepares the final checkpoint before Phase 50',
          ].map((item, i) => (
      ...
      {latestBatch && (
        <div className="bg-secondary/10 border border-border/60 rounded-lg overflow-hidden">
          ...
          <div className="px-4 py-2 border-t border-border/40 flex items-center gap-1.5">
            ...
            <span className="text-[8px] text-primary/80">Stored in localStorage key: <span className="font-mono">{ACTIVATION_LOCKS_KEY}</span></span>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-2 px-3 py-2 bg-primary/5 border border-primary/20 rounded text-[8px] text-primary/80">
        ...
        Local-only activation lock. No fetch, no OpenClaw calls, no backend calls, no secret values, no execution, no dispatch.
      </div>
    </div>
  );
}