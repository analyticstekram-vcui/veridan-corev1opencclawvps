/**
 * BrowserObservationProposalFinalLockPanel — Local-only Phase 16 Final Lock
 * Verifies and locks the proposal queue, review ledger, and readiness gate.
...
const LOCK_KEY     = 'openclawBrowserObservationProposalFinalLock';
const LOCK_NAME    = 'OPENCLAW_BROWSER_OBSERVATION_PROPOSAL_FINAL_LOCK';
const PHASE_NAME   = 'PHASE_16_BROWSER_OBSERVATION_PROPOSAL_QUEUE';
const FINAL_WARNING = 'This Phase 16 lock is local-only and non-executable. It does not authorize browser automation, clicking, typing, credentials, trading, broker actions, wallet actions, money movement, command dispatch, scheduler, polling, or backend calls.';
...
const SOURCE_LABELS = {
  phase15LockPresent:   { label: 'Phase 15 Final Lock', key: KEYS.phase15Lock },
  proposalsPresent:     { label: 'Proposals',           key: KEYS.proposals },
  reviewLedgerPresent:  { label: 'Review Ledger',       key: KEYS.ledger },
...
function buildPresence() {
  return {
    ...
    proposalsPresent:     !!localStorage.getItem(KEYS.proposals),
    reviewLedgerPresent:  !!localStorage.getItem(KEYS.ledger),
...
function runLock() {
  ...
  const gate      = loadJSON(KEYS.gate, null);

  const presence = buildPresence();

  const readinessDecision = gate?.readinessDecision ?? null;
  ...
  const lockChecks = {
    phase15FinalLockPresent:            presence.phase15LockPresent,
    proposalsPresent:                   proposals.length > 0,
    reviewLedgerPresent:                ledger.length > 0,
    readinessGatePresent:               !!gate,
  ...
  const allChecksPass = Object.values(lockChecks).every(Boolean);

  let lockStatus;
  if (blockedApprovedAsDesign || anyUnsafeReview || readinessDecision === 'BLOCKED_BY_POLICY') {
    lockStatus = 'BLOCKED_BY_POLICY';
  } else if (
    allChecksPass &&
    (readinessDecision === 'READY_FOR_DESIGN_REVIEW' || readinessDecision === 'HOLD_FOR_REVIEW')
  ) {
  ...
  return {
    ...
    sourcePhase15LockPresent: presence.phase15LockPresent,
    proposalCount:           proposals.length,
    reviewCount:             ledger.length,
    readinessGatePresent:    !!gate,
    readinessDecision:       readinessDecision ?? 'NOT_EVALUATED',
    lockChecks,
    lockStatus,
...
export default function BrowserObservationProposalFinalLockPanel() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Header */}
      <div>
        <div className="text-[11px] uppercase tracking-widest text-slate-400 font-semibold mb-0.5">Phase 16 · Browser Observation Proposal Final Lock</div>
        <div className="text-[13px] font-bold text-foreground flex items-center gap-2">
          <Lock className="w-4 h-4 text-primary" /> Browser Observation Proposal Final Lock
        </div>
        <div className="text-[9px] text-slate-500 mt-0.5">Local-only Phase 16 final lock. No automation, execution, dispatch, or backend calls.</div>
      </div>
      ...
      {/* Count summary + readiness decision (after lock generated) */}
      {lock && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[8px]">
          {[
            { k: 'Proposals',         v: lock.proposalCount },
            { k: 'Reviews',           v: lock.reviewCount },
            { k: 'Readiness Gate',    v: lock.readinessGatePresent ? 'PRESENT' : 'MISSING', vc: lock.readinessGatePresent ? 'text-primary font-bold' : 'text-slate-500 font-bold' },
            { k: 'Readiness Decision',v: lock.readinessDecision,                             vc: lock.readinessDecision === 'READY_FOR_DESIGN_REVIEW' ? 'text-primary font-bold' : lock.readinessDecision === 'BLOCKED_BY_POLICY' ? 'text-destructive font-bold' : 'text-amber-500 font-bold' },
          ].map(({ k, v, vc }) => (
        ...
        return (
          <div className={`border rounded-lg p-4 flex items-center gap-3 ${statusCfg.bg}`}>
            ...
            <div>
              ...
              <div className={`text-[14px] font-bold uppercase tracking-wide mt-0.5 ${statusCfg.color}`}>
                {lock.lockStatus}
              </div>
        ...
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          ...
          <div className="divide-y divide-border/30">
            {Object.entries(lock.lockChecks).map(([key, value]) => {
              const isBlocked = !value && lock.lockStatus === 'BLOCKED_BY_POLICY';
              ...
              return (
                <div key={key} className="flex items-center gap-3 px-4 py-2.5">
                  ...
                  <span className={`text-[7px] font-bold px-1.5 py-0.5 rounded border shrink-0 ${badgeClass}`}>
                    {value ? 'PASS' : isBlocked ? 'BLOCKED' : 'HOLD'}
                  </span>
                </div>
              );
            })}
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          ...
          {copied ? 'Copied!' : 'Copy Phase 16 Final Lock JSON'}
        </button>
        <button
          type="button"
          onClick={handleClear}
        ...
        >
          ...
          Clear Local Phase 16 Final Lock
        </button>
      </div>
      ...
      {lock && (
        <div className="bg-secondary/10 border border-border/60 rounded-lg overflow-hidden">
          <div className="flex items-center justify-between px-4 py-2 bg-secondary/20 border-b border-border">
            <span className="text-[9px] uppercase tracking-widest text-slate-400 font-semibold">Phase 16 Final Lock — JSON Preview</span>
