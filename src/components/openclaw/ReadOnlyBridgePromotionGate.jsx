/**
 * ReadOnlyBridgePromotionGate
 * Local-only read-only monitoring promotion gate.
 *
 * SAFETY CONTRACT:
 *   - No network calls, no OpenClaw calls, no browser automation
 *   - No dispatch, no execution, no trading, no credentials, no money movement
 *   - Reads/writes localStorage only
 *   - PREVIEW_ONLY / READ_ONLY / LOCKED
 */
import React, { useState, useEffect, useCallback } from 'react';
import { CheckCircle2, AlertTriangle, XCircle, Copy, ShieldCheck, RefreshCw, FileJson, Lock } from 'lucide-react';

const SOURCE_KEYS = {
  integrityCheckpoints:    'openclawBridgeIntegrityCheckpoints',
  auditDashboards:         'openclawBridgeAuditReportDashboards',
  resultEvidenceExports:   'openclawBridgeCallResultEvidenceExports',
  bridgeCalls:             'openclawControlledReadOnlyRouteBridgeCalls',
  approvalPackets:         'openclawReadOnlyRouteApprovalPackets',
  handoffPackets:          'openclawOperatorHandoffPackets',
  auditTrail:              'openclawAuditTrail',
};
const GATE_KEY = 'openclawReadOnlyBridgePromotionGates';

const EXPLICITLY_BLOCKED = [
  'command dispatch',
  'browser execution',
  'trading',
  'broker execution',
  'credential entry',
  'wallet actions',
  'money movement',
  'mutation endpoints',
];

const REQUIRED_EVIDENCE = [
  'Bridge Integrity Checkpoint',
  'Bridge Audit Report Dashboard',
  'Bridge Call Result Evidence Export',
  'Controlled Read-Only Route Bridge Call',
  'Read-Only Route Approval Packet',
];

function tryAppendAudit(entry) {
  try { import('@/lib/proposalStore').then(m => m.appendAudit?.(entry)).catch(() => {}); } catch {}
}

function loadJSON(key, fallback = []) {
  try { return JSON.parse(localStorage.getItem(key) || JSON.stringify(fallback)); } catch { return fallback; }
}

function saveGate(gate) {
  try {
    const all = loadJSON(GATE_KEY, []);
    const deduped = [gate, ...all.filter(g => g.gateId !== gate.gateId)];
    localStorage.setItem(GATE_KEY, JSON.stringify(deduped.slice(0, 30)));
  } catch {}
}

function evaluatePromotionDecision() {
  const integrityCheckpoints = loadJSON(SOURCE_KEYS.integrityCheckpoints, []);
  const auditDashboards = loadJSON(SOURCE_KEYS.auditDashboards, []);
  const resultEvidenceExports = loadJSON(SOURCE_KEYS.resultEvidenceExports, []);
  const bridgeCalls = loadJSON(SOURCE_KEYS.bridgeCalls, []);
  const approvalPackets = loadJSON(SOURCE_KEYS.approvalPackets, []);

  const latestIntegrityCheckpoint = integrityCheckpoints[0];
  const latestAuditDashboard = auditDashboards[0];
  const latestResultEvidence = resultEvidenceExports[0];
  const latestBridgeCall = bridgeCalls[0];
  const latestApprovalPacket = approvalPackets[0];

  const successfulBridgeCalls = bridgeCalls.filter(c => 
    (c.gatewayReachable ?? c.online ?? c.reachable ?? false) && 
    !c.executionAttempted && 
    !c.secretExposed && 
    !c.dispatchAllowed
  ).length;

  // Check for safety failures
  const hasSafetyFailures = 
    (latestBridgeCall?.executionAttempted === true) ||
    (latestBridgeCall?.dispatchAllowed === true) ||
    (latestBridgeCall?.openClawCommandSent === true) ||
    (latestBridgeCall?.browserToolUsed === true) ||
    (latestBridgeCall?.secretExposed === true) ||
    (latestAuditDashboard?.secretExposed === true);

  const integrityCheckpointPass = latestIntegrityCheckpoint?.overallStatus === 'PASS';
  const auditDashboardPass = latestAuditDashboard?.overallStatus === 'PASS';
  const hasSuccessfulBridgeCall = successfulBridgeCalls > 0;
  const hasApprovalPacket = !!latestApprovalPacket;

  // Determine promotion decision
  if (hasSafetyFailures) {
    return 'BLOCKED_BY_SAFETY_FAILURE';
  }

  if (integrityCheckpointPass && auditDashboardPass && hasSuccessfulBridgeCall && hasApprovalPacket) {
    return 'APPROVED_FOR_READ_ONLY_MONITORING';
  }

  if (!hasSafetyFailures && (!integrityCheckpointPass || !auditDashboardPass || !hasSuccessfulBridgeCall || !hasApprovalPacket)) {
    return 'HOLD_FOR_MORE_EVIDENCE';
  }

  return 'HOLD_FOR_MORE_EVIDENCE';
}

function buildGate() {
  const integrityCheckpoints = loadJSON(SOURCE_KEYS.integrityCheckpoints, []);
  const auditDashboards = loadJSON(SOURCE_KEYS.auditDashboards, []);
  const resultEvidenceExports = loadJSON(SOURCE_KEYS.resultEvidenceExports, []);
  const bridgeCalls = loadJSON(SOURCE_KEYS.bridgeCalls, []);
  const approvalPackets = loadJSON(SOURCE_KEYS.approvalPackets, []);

  const latestIntegrityCheckpoint = integrityCheckpoints[0];
  const latestAuditDashboard = auditDashboards[0];
  const latestResultEvidence = resultEvidenceExports[0];
  const latestBridgeCall = bridgeCalls[0];
  const latestApprovalPacket = approvalPackets[0];

  const successfulBridgeCalls = bridgeCalls.filter(c => 
    (c.gatewayReachable ?? c.online ?? c.reachable ?? false) && 
    !c.executionAttempted && 
    !c.secretExposed && 
    !c.dispatchAllowed
  ).length;

  const requiredEvidenceChecks = REQUIRED_EVIDENCE.map(name => ({
    name,
    present: 
      (name === 'Bridge Integrity Checkpoint' && !!latestIntegrityCheckpoint) ||
      (name === 'Bridge Audit Report Dashboard' && !!latestAuditDashboard) ||
      (name === 'Bridge Call Result Evidence Export' && !!latestResultEvidence) ||
      (name === 'Controlled Read-Only Route Bridge Call' && !!latestBridgeCall) ||
      (name === 'Read-Only Route Approval Packet' && !!latestApprovalPacket),
  }));

  const safetyFailures = [
    { name: 'executionAttempted',     value: latestBridgeCall?.executionAttempted ?? false },
    { name: 'dispatchAllowed',        value: latestBridgeCall?.dispatchAllowed ?? false },
    { name: 'openClawCommandSent',    value: latestBridgeCall?.openClawCommandSent ?? false },
    { name: 'browserToolUsed',        value: latestBridgeCall?.browserToolUsed ?? false },
    { name: 'secretExposed',          value: (latestBridgeCall?.secretExposed ?? false) || (latestAuditDashboard?.secretExposed ?? false) },
  ];

  const safetyAssertions = [
    { key: 'previewOnly',              value: true,                              pass: true },
    { key: 'readOnly',                 value: true,                              pass: true },
    { key: 'gatewayMode',              value: 'READ_ONLY',                      pass: true },
    { key: 'executionMode',            value: 'DISABLED',                       pass: true },
    { key: 'executionLock',            value: 'LOCKED',                         pass: true },
    { key: 'dispatchAllowed',          value: false,                            pass: true },
    { key: 'commandDispatchAllowed',   value: false,                            pass: true },
    { key: 'executionAllowed',         value: false,                            pass: true },
    { key: 'tradingDisabled',          value: true,                             pass: true },
    { key: 'brokerActionsDisabled',    value: true,                             pass: true },
    { key: 'credentialEntryDisabled',  value: true,                             pass: true },
    { key: 'walletActionsDisabled',    value: true,                             pass: true },
    { key: 'moneyMovementDisabled',    value: true,                             pass: true },
    { key: 'mutationEndpointsBlocked', value: true,                             pass: true },
  ];

  const promotionDecision = evaluatePromotionDecision();

  const gateId = 'robpg-' + Date.now().toString(36) + '-' + Math.random().toString(36).slice(2, 6);

  return {
    gateId,
    createdAt:                  new Date().toISOString(),
    phase:                      'READ_ONLY_BRIDGE_PROMOTION_GATE',
    currentStatus:              'EVALUATING_READ_ONLY_BRIDGE',
    promotionDecision,
    gatewayMode:                'READ_ONLY',
    executionMode:              'DISABLED',
    executionLock:              'LOCKED',
    dispatchAllowed:            false,
    commandDispatchAllowed:     false,
    executionAllowed:           false,
    approvedScope:              'READ_ONLY_STATUS_MONITORING_ONLY',
    explicitlyBlocked:          EXPLICITLY_BLOCKED,
    requiredEvidenceChecks,
    safetyFailures,
    sourceDiagnostics: {
      integrityCheckpointPresent:  !!latestIntegrityCheckpoint,
      auditDashboardPresent:       !!latestAuditDashboard,
      resultEvidencePresent:       !!latestResultEvidence,
      bridgeCallPresent:           !!latestBridgeCall,
      approvalPacketPresent:       !!latestApprovalPacket,
      successfulBridgeCalls:       successfulBridgeCalls,
      totalBridgeCalls:            bridgeCalls.length,
    },
    safetyAssertions,
    nextRecommendedPhase: promotionDecision === 'APPROVED_FOR_READ_ONLY_MONITORING' 
      ? 'FINALIZE_READ_ONLY_MONITORING_BASELINE' 
      : promotionDecision === 'HOLD_FOR_MORE_EVIDENCE'
      ? 'GENERATE_ADDITIONAL_BRIDGE_EVIDENCE'
      : 'INVESTIGATE_SAFETY_FAILURE',
    note: 'Promotion gate evaluation only. No dispatch. No execution. No network calls. Read-only monitoring approved scope only.',
  };
}

function CopyButton({ data }) {
  const [copied, setCopied] = useState(false);
  const handle = () => {
    navigator.clipboard.writeText(JSON.stringify(data, null, 2));
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
  };
  return (
    <button type="button" onClick={handle}
      className="flex items-center gap-1.5 px-2.5 py-1.5 text-[9px] border border-border text-slate-400 hover:text-foreground hover:bg-secondary/50 rounded font-bold transition-colors">
      {copied ? <CheckCircle2 className="w-3 h-3 text-primary" /> : <Copy className="w-3 h-3" />}
      {copied ? 'Copied!' : 'Copy Promotion Gate JSON'}
    </button>
  );
}

export default function ReadOnlyBridgePromotionGate({ refreshTrigger }) {
  const [gate, setGate] = useState(null);
  const [showJSON, setShowJSON] = useState(false);

  const evaluate = useCallback(() => {
    const g = buildGate();
    saveGate(g);
    tryAppendAudit({
      event:             'read_only_bridge_promotion_gate_evaluated',
      gateId:            g.gateId,
      promotionDecision: g.promotionDecision,
      approvedScope:     g.approvedScope,
      note: `Promotion gate evaluated (${g.gateId}). Decision: ${g.promotionDecision}. Approved scope: ${g.approvedScope}. No dispatch. No execution.`,
    });
    setGate(g);
  }, []);

  useEffect(() => { evaluate(); }, [refreshTrigger, evaluate]);

  const DECISION_STYLE = {
    APPROVED_FOR_READ_ONLY_MONITORING: { color: 'text-primary',     bg: 'bg-primary/5 border-primary/30',         icon: CheckCircle2,  label: 'APPROVED FOR READ-ONLY MONITORING' },
    HOLD_FOR_MORE_EVIDENCE:             { color: 'text-amber-500',   bg: 'bg-amber-500/5 border-amber-500/20',     icon: AlertTriangle, label: 'HOLD FOR MORE EVIDENCE' },
    BLOCKED_BY_SAFETY_FAILURE:          { color: 'text-destructive', bg: 'bg-destructive/5 border-destructive/20', icon: XCircle,       label: 'BLOCKED BY SAFETY FAILURE' },
  };

  const style = DECISION_STYLE[gate?.promotionDecision] || DECISION_STYLE.HOLD_FOR_MORE_EVIDENCE;
  const Icon = style.icon;

  return (
    <div className="space-y-4">

      {/* Header */}
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <div className="text-[11px] uppercase tracking-widest text-slate-400 font-semibold mb-0.5">Phase: Promotion Gate</div>
          <div className="text-[13px] font-bold text-foreground">Read-Only Bridge Promotion Gate</div>
          <div className="text-[9px] text-slate-500 mt-0.5">Evaluates readiness for read-only monitoring promotion. No dispatch. No execution.</div>
        </div>
        <button type="button" onClick={evaluate}
          className="flex items-center gap-1.5 px-2.5 py-1.5 text-[9px] border border-border text-slate-400 hover:bg-secondary/50 rounded font-bold transition-colors">
          <RefreshCw className="w-3 h-3" /> Evaluate Gate
        </button>
      </div>

      {/* Safety banner */}
      <div className="flex items-start gap-2 px-3 py-2 bg-amber-500/5 border border-amber-500/20 rounded text-[9px] text-amber-500/90">
        <ShieldCheck className="w-3.5 h-3.5 shrink-0 mt-0.5" />
        <span><span className="font-bold">GATE_EVALUATION_ONLY / READ_ONLY / LOCKED</span> — Promotion decision. No dispatch. No execution. Read-only monitoring scope only.</span>
      </div>

      {gate && (
        <>
          {/* Promotion decision banner */}
          <div className={`border rounded-lg p-4 space-y-2 ${style.bg}`}>
            <div className="flex items-center gap-3">
              <Icon className={`w-5 h-5 shrink-0 ${style.color}`} />
              <div>
                <div className={`text-[13px] font-bold uppercase tracking-wide ${style.color}`}>
                  {style.label}
                </div>
                <div className={`text-[9px] mt-1 ${style.color === 'text-destructive' ? 'text-destructive/80' : style.color === 'text-amber-500' ? 'text-amber-500/80' : 'text-primary/80'}`}>
                  Approved Scope: {gate.approvedScope}
                </div>
              </div>
            </div>
          </div>

          {/* Summary cards */}
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2">
            {[
              { label: 'Promotion Decision',    value: gate.promotionDecision.split('_').slice(0, 2).join(' '), color: style.color },
              { label: 'Gateway Mode',          value: gate.gatewayMode,                color: 'text-primary font-bold' },
              { label: 'Execution Mode',        value: gate.executionMode,              color: 'text-primary font-bold' },
              { label: 'Execution Lock',        value: gate.executionLock,              color: 'text-primary font-bold' },
              { label: 'Dispatch Allowed',      value: String(gate.dispatchAllowed),    color: 'text-destructive font-bold' },
              { label: 'Execution Allowed',     value: String(gate.executionAllowed),   color: 'text-destructive font-bold' },
              { label: 'Successful Calls',      value: gate.sourceDiagnostics.successfulBridgeCalls, color: 'text-primary font-bold' },
              { label: 'Safety Failures',       value: gate.safetyFailures.filter(f => f.value).length, color: gate.safetyFailures.some(f => f.value) ? 'text-destructive font-bold' : 'text-primary font-bold' },
            ].map(c => (
              <div key={c.label} className="bg-card border border-border/60 rounded-lg px-2.5 py-2">
                <div className="text-[7px] uppercase tracking-widest text-slate-500 font-semibold mb-0.5">{c.label}</div>
                <div className={`text-[11px] font-bold ${c.color}`}>{c.value}</div>
              </div>
            ))}
          </div>

          {/* Required evidence checklist */}
          <div className="bg-secondary/10 border border-border/60 rounded-lg p-3">
            <div className="text-[9px] uppercase tracking-widest text-slate-400 font-semibold mb-2">Required Evidence Checklist</div>
            <div className="space-y-1.5">
              {gate.requiredEvidenceChecks.map((check, i) => (
                <div key={i} className="flex items-center gap-2 text-[8px]">
                  {check.present ? (
                    <CheckCircle2 className="w-3.5 h-3.5 text-primary shrink-0" />
                  ) : (
                    <div className="w-3.5 h-3.5 rounded-full border-2 border-slate-500 shrink-0" />
                  )}
                  <span className={check.present ? 'text-slate-300 font-semibold' : 'text-slate-500'}>{check.name}</span>
                </div>
              ))}
            </div>
          </div>

          {/* Explicitly blocked capabilities */}
          <div className="bg-destructive/5 border border-destructive/20 rounded-lg p-3">
            <div className="text-[9px] uppercase tracking-widest text-destructive font-semibold mb-2">Explicitly Blocked Capabilities</div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
              {gate.explicitlyBlocked.map((cap, i) => (
                <div key={i} className="flex items-center gap-1.5 px-2 py-1 bg-destructive/10 border border-destructive/30 rounded">
                  <Lock className="w-3 h-3 text-destructive shrink-0" />
                  <span className="text-[7px] font-bold text-destructive">{cap}</span>
                </div>
              ))}
            </div>
          </div>

          {/* Safety assertions */}
          <div className="bg-card border border-border rounded-lg overflow-hidden">
            <div className="px-4 py-2 bg-secondary/10 border-b border-border">
              <span className="text-[9px] uppercase tracking-widest text-slate-400 font-semibold">
                Safety Assertions — {gate.safetyAssertions.filter(a => a.pass).length}/{gate.safetyAssertions.length} PASS
              </span>
            </div>
            <div className="px-4 py-3 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-y-2 gap-x-4">
              {gate.safetyAssertions.map(a => (
                <div key={a.key} className="flex items-center gap-1.5">
                  {a.pass
                    ? <CheckCircle2 className="w-3 h-3 text-primary shrink-0" />
                    : <div className="w-3 h-3 rounded-full bg-destructive shrink-0" />}
                  <span className="font-mono text-[7px] text-slate-400">{a.key}:</span>
                  <span className={`text-[7px] font-bold ${a.pass ? 'text-primary' : 'text-destructive'}`}>
                    {String(a.value)}
                  </span>
                </div>
              ))}
            </div>
          </div>

          {/* JSON preview */}
          <details className="bg-secondary/10 border border-border/60 rounded-lg">
            <summary className="px-3 py-2 cursor-pointer hover:bg-secondary/20 transition-colors flex items-center gap-2 text-[9px] font-semibold text-slate-400 uppercase tracking-widest">
              <FileJson className="w-3.5 h-3.5" /> Promotion Gate JSON
            </summary>
            {showJSON && (
              <div className="px-3 py-2 border-t border-border/60 bg-secondary/20">
                <pre className="text-[7px] font-mono text-slate-300 overflow-auto max-h-48 whitespace-pre-wrap break-words">
                  {JSON.stringify(gate, null, 2)}
                </pre>
              </div>
            )}
          </details>
          <button type="button" onClick={() => setShowJSON(!showJSON)}
            className="text-[8px] text-slate-500 hover:text-slate-300 underline">
            {showJSON ? 'Hide' : 'Show'} JSON
          </button>

          {/* Gate ID + timestamp */}
          <div className="flex flex-wrap gap-4 text-[8px] text-slate-500">
            <span className="flex items-center gap-1.5"><Lock className="w-3 h-3" /><span className="font-mono">{gate.gateId}</span></span>
            <span>{new Date(gate.createdAt).toLocaleString()}</span>
          </div>

          {/* Actions */}
          <div className="flex flex-wrap gap-2">
            <CopyButton data={gate} />
            <button type="button" onClick={evaluate}
              className="flex items-center gap-1.5 px-2.5 py-1.5 text-[9px] border border-primary/40 text-primary bg-primary/5 hover:bg-primary/15 rounded font-bold transition-colors">
              <Lock className="w-3 h-3" /> Evaluate Promotion Gate
            </button>
          </div>
        </>
      )}

      {/* Safety footer */}
      <div className="flex items-center gap-2 px-3 py-2 bg-primary/5 border border-primary/20 rounded text-[8px] text-primary/80">
        <CheckCircle2 className="w-3 h-3 shrink-0" />
        Promotion gate approves read-only monitoring only. No command dispatch. No execution. No trading. No credentials. No money movement.
      </div>
    </div>
  );
}